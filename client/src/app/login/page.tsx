...
const Login = () => {
    ...
    const handlesubmit=async(event)=>{
        ...
        try{
            const res=axios.post('http://localhost:5000/api/auth/login',{email:email,password:password})
  ...
  return (
    <div className="flex h-screen justify-center items-center bg-lime-100">
      <form action="" method="post" className="flex flex-col  gap-5 p-10 bg-red-800 w-1/2 rounded-xl" onSubmit={handlesubmit}>
        <h2 className="text-center text-white text-3xl font-bold">Login Page</h2>
        ...
        <button className="bg-blue-200  p-3 hover:bg-blue-100 rounded-full text-xl " type="submit">Login here</button>
      </form>
    </div>
  );
};
